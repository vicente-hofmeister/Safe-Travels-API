...
const locationSeeds: LocationSeed[] = [
  ...
  // ── 2026-05-10 — Expedição Litoral Gaúcho (rota da trip de grupo) ───────
  { username: "root",  latitude: -29.7428, longitude: -49.9987, accuracyMeters: 6,  capturedAt: "2026-05-10T10:00:00Z", tag: "litoral_root_1" },
  { username: "root",  latitude: -29.7435, longitude: -49.9993, accuracyMeters: 5,  capturedAt: "2026-05-10T10:20:00Z", tag: "litoral_root_2" },
  { username: "alice", latitude: -29.7440, longitude: -49.9998, accuracyMeters: 7,  capturedAt: "2026-05-10T10:05:00Z", tag: "litoral_alice_1" },
  { username: "alice", latitude: -29.7447, longitude: -50.0005, accuracyMeters: 6,  capturedAt: "2026-05-10T10:25:00Z", tag: "litoral_alice_2" },
  { username: "bob",   latitude: -29.3418, longitude: -49.7308, accuracyMeters: 9,  capturedAt: "2026-05-10T11:00:00Z", tag: "litoral_bob_1" },
  { username: "bob",   latitude: -29.3425, longitude: -49.7315, accuracyMeters: 8,  capturedAt: "2026-05-10T11:20:00Z", tag: "litoral_bob_2" },

  // ── 2026-05-03 — Rota das Missões (rota da trip solo do root) ────────────
  ...
  { username: "root",  latitude: -28.5590, longitude: -54.5515, accuracyMeters: 7,  capturedAt: "2026-05-03T09:10:00Z", tag: "missoes_root_1" },
  { username: "root",  latitude: -28.5597, longitude: -54.5522, accuracyMeters: 9,  capturedAt: "2026-05-03T09:30:00Z", tag: "missoes_root_2" },
...
async function seed() {
  ...
  // ── Reset (mantém users) ───────────────────────────────────────────────────
  console.log("Resetting groups, trips and location data...");
  await query(`
    TRUNCATE trips, location_event_groups, location_events, group_members, groups
  ...
  // Mapa usuario → [groupIds] para linking de location events
  ...
  const locationEventIdByTag = new Map<string, number>();
  ...
  for (const loc of locationSeeds) {
    ...
    if (loc.tag) locationEventIdByTag.set(loc.tag, locationEventId);
    ...
    const groupLabels = groupIds.length > 0
      ? groups
          .filter((g) => groupIds.includes(groupIdBySlug.get(g.slug)!))
          .map((g) => g.slug)
          .join(", ")
      : "sem grupo";
  ...
  const rootId  = userIdByUsername.get("root")!;
  const aliceId = userIdByUsername.get("alice")!;
  const bobId   = userIdByUsername.get("bob")!;
  const expedicaoLitoralId = groupIdBySlug.get("expedicao_litoral")!;

  // Trip 1 — individual, encerrada, com rota (Missões)
  const tripSoloResult = await query<{ trip_id: string }>(
    `INSERT INTO trips (name, description, owner_id, group_id, started_at, ended_at)
     VALUES ($1, $2, $3, NULL, $4::timestamptz, $5::timestamptz)
     RETURNING trip_id`,
    [
      "Mochilão nas Missões",
      "Viagem individual pelas missões jesuíticas do RS.",
      rootId,
      "2026-05-03T08:00:00Z",
      "2026-05-03T20:00:00Z",
    ],
  );
  const tripSoloId = tripSoloResult.rows[0]?.trip_id;
  if (!tripSoloId) throw new Error("Failed to create solo trip");

  await query(`INSERT INTO user_trips (trip_id, user_id) VALUES ($1, $2)`, [tripSoloId, rootId]);

  const missoesTags = ["missoes_root_1", "missoes_root_2"];
  for (const tag of missoesTags) {
    const eventId = locationEventIdByTag.get(tag);
    if (eventId) {
      await query(
        `INSERT INTO location_event_trips (location_event_id, trip_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
        [eventId, tripSoloId],
      );
    }
  }
  console.log(`  "Mochilão nas Missões" → ${tripSoloId} (individual, encerrada, ${missoesTags.length} pontos)`);

  // Trip 2 — de grupo, ativa, com rota (Expedição Litoral)
  const tripGrupoResult = await query<{ trip_id: string }>(
    `INSERT INTO trips (name, description, owner_id, group_id, started_at)
     VALUES ($1, $2, $3, $4, $5::timestamptz)
     RETURNING trip_id`,
    [
      "Expedição Litoral Gaúcho 2026",
      "Percorrendo as praias do litoral gaúcho com o grupo.",
      rootId,
      expedicaoLitoralId,
      "2026-05-10T09:00:00Z",
    ],
  );
  const tripGrupoId = tripGrupoResult.rows[0]?.trip_id;
  if (!tripGrupoId) throw new Error("Failed to create group trip");

  for (const userId of [rootId, aliceId, bobId]) {
    await query(
  ...
  const litoralTags = [
    "litoral_root_1", "litoral_root_2",
    "litoral_alice_1", "litoral_alice_2",
    "litoral_bob_1",  "litoral_bob_2",
  ];
  for (const tag of litoralTags) {
    const eventId = locationEventIdByTag.get(tag);
    if (eventId) {
      await query(
        `INSERT INTO location_event_trips (location_event_id, trip_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
  ...
  console.log(`  "Expedição Litoral Gaúcho 2026" → ${tripGrupoId} (grupo, ativa, ${litoralTags.length} pontos)`);
